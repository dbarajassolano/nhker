...
class Api(NHKEasy):
    ...
    def download_text_by_priority(self, priority: int) -> tuple[str, str]:
        news = self.top_news
        if priority >= len(news):
            raise ValueError(f'Requested index cannot exceed {len(news)}')
        else:
            [id_, title] = [
                news[priority][a] for a in ("news_id", "title")
            ]
            article = Article(id_)
            text = article.get_text(furigana=False, html_output=False)
